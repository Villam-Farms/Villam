...
export async function searchUsers(
  ...
  return apiRequest<SearchUser[]>(`/users/search?${query.toString()}`, {
    accessToken,
  });
}
...
export async function listFollowers(
  ...
  if (q.trim()) query.set("q", q.trim());
  query.set("limit", String(limit));
  return apiRequest<SearchUser[]>(`/followers?${query.toString()}`, {
    accessToken,
  });
}
...
export async function listFollowing(
  ...
  if (q.trim()) query.set("q", q.trim());
  query.set("limit", String(limit));
  return apiRequest<SearchUser[]>(`/following?${query.toString()}`, {
    accessToken,
  });
}
...
export async function unfollowUser(
  ...
  await apiRequest<void>(`/follow/${encodeURIComponent(followingId)}`, {
    method: "DELETE",
    accessToken,
  });
}
